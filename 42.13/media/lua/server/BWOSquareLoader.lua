...
local processSquare = function(square,player)
        ...
        if not md.BWO.dbs then
            ...
            if BanditUtils.HasZoneType(x, y, z, "TownZone") then
                ...
                if density > 0 and shouldPlace(x, y, density, seed) then
                    ...
                    local fakeZombie = player:getCell():getFakeZombieForHit()
...
local processLocationEvents = function(ticks)
    ...
    local gmd = BWOGMD.Get()
    local tab = gmd and gmd.PlaceEvents or {}
        ...
        if vehicle then
            ...
            if not md.BWO.wasRepaired then
                ...
                if scriptName:embodies("Burnt") or scriptName:embodies("Smashed") or not engine or engine:getCondition() < 50 then
                    ...
                    if key then
                        local result = BWOUtils.GetClosestBanditVehicle(vehicle)
                        if result.id then
                            local bandit = BWOZombie.GetInstanceById(result.id)
                            if bandit then
