...
BWOBuildings.FindBuildingWithRoom = function(bsearch)
    ...
    for _, player in ipairs(players) do
        ...
        local rooms = cell and cell:getRoomList()
        if rooms then
            for i = 0, rooms:size() - 1 do
                ...
                if building and building:containsRoom(bsearch) and not BWOBuildings.IsEventBuilding(building, "home") then
                    local def = building:getDef()
                    if def and math.abs(def:getX() - player:getX()) < 70 and math.abs(def:getX2() - player:getX()) < 70 and
    ...
    return buildings[ZombRand(#buildings) + 1]
end
